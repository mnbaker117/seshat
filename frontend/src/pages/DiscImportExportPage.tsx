...
try{const d:any=await api.post("/discovery/books/import-preview",{urls:lines});setResults(d.results||[]);setProgress("")}catch(e){setProgress("Error fetching books")}setFetching(false)};

const addBooks=async(books:any[])=>{setAdding(true);setAddResult(null);
try{const d=await api.post("/discovery/books/import-add",{books});setAddResult(d);
